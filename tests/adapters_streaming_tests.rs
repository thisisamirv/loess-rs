...
//!
//! The Streaming adapter provides chunked LOESS processing for large datasets,
//! designed for:
...
fn test_streaming_invalid_overlap() {
    ...
    assert!(
        matches!(result, Err(LoessError::InvalidOverlap { .. })),
...
fn test_streaming_invalid_fraction() {
    ...
    assert!(
        matches!(result, Err(LoessError::InvalidFraction(_))),
...
/// Test StreamingLoessBuilder default values.
#[test]
