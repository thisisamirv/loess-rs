...
//!
//! These tests verify the validation functions used in LOESS for:
...
fn test_validate_invalid_fraction() {
    ...
    assert!(
        matches!(
            ...
            Err(LoessError::InvalidFraction(_))
        ),
...
fn test_validate_tolerance() {
    ...
    assert!(
        matches!(
            ...
            Err(LoessError::InvalidTolerance(t)) if t == 0.0
        ),
    ...
    assert!(
        matches!(
            ...
            Err(LoessError::InvalidTolerance(t)) if t == -1.0
        ),
    ...
    assert!(
        matches!(
            ...
            Err(LoessError::InvalidTolerance(t)) if t.is_nan()
        ),
...
fn test_validate_tolerance_boundaries() {
    ...
    assert!(
        matches!(
            Validator::validate_tolerance(0.0),
            Err(LoessError::InvalidTolerance(_))
        ),
        "Zero tolerance should error"
    );
