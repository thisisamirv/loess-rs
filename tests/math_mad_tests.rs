...
fn test_mad_even_length() {
    ...
    let mad = compute_mad(&mut residuals);
...
fn test_mad_scale_invariance() {
    ...
    let mad1 = compute_mad(&mut residuals.clone());

    let mut scaled: Vec<f64> = residuals.iter().map(|&x| x * 10.0).collect();
    let mad2 = compute_mad(&mut scaled);
...
fn test_mad_unsorted_input() {
    ...
    let mad_unsorted = compute_mad(&mut unsorted);

    let mut sorted = vec![1.0f64, 2.0, 3.0, 4.0];
    let mad_sorted = compute_mad(&mut sorted);
