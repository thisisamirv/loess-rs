#![cfg(feature = "dev")]
//! Tests for LOESS output structures.
//!
//! These tests verify the LoessResult structure and its methods:
...
/// Helper trait for testing LoessResult methods.
///
/// These methods are only used in tests and are not part of the public API.
trait LoessResultTestExt<T: Float> {
...
impl<T: Float> LoessResultTestExt<T> for LoessResult<T> {
    fn smoothed(&self) -> &[T] {
        &self.y
    }
...
fn test_display_with_all_fields() {
    ...
    assert!(output.contains("LOESS Diagnostics"));
