...
//! 5. **Result Helpers** - Utility methods on LoessResult
...
fn test_builder_converts_to_streaming() {
    let sb = Loess::<f64>::new().fraction(0.5).adapter(Streaming);
    assert!(
        sb.build().is_ok(),
...
fn test_builder_converts_to_online() {
    let ob = Loess::<f64>::new().fraction(0.5).adapter(Online);
    assert!(
        ob.build().is_ok(),
...
fn test_validate_empty_cv_fractions() {
    ...
    assert!(
        matches!(res, Err(LoessError::InvalidFraction(_))),
...
fn test_validate_invalid_confidence_level() {
    ...
    assert!(matches!(got, Err(LoessError::InvalidIntervals(_))));

    // Verify validator also rejects
    match Validator::validate_interval_level(1.5) {
        Err(LoessError::InvalidIntervals(val)) => {
...
fn test_loess_rs_error_display() {
    let errs = [
        ...
        LoessError::InvalidOverlap {
            overlap: 10,
            chunk_size: 10,
        },
        LoessError::InvalidWindowCapacity { got: 2, min: 3 },
        LoessError::InvalidMinPoints {
            got: 1,
            window_capacity: 10,
        },
        LoessError::UnsupportedFeature {
            adapter: "Streaming",
            feature: "test",
        },
    ];
...
fn test_diagnostics_display() {
    ...
    assert!(s.contains("LOESS Diagnostics"));
...
/// Test LoessResult helper methods.
///
...
fn test_batch_keeps_options() {
    let base = Loess::<f64>::new().fraction(0.42).iterations(2);
...
fn test_builder_clone_independence() {
    let builder1 = Loess::new().fraction(0.5).iterations(2);
...
fn test_builder_parameter_override() {
    ...
    match result {
        Err(LoessError::DuplicateParameter { parameter }) => {
...
fn test_builder_default_after_custom() {
    let _custom_builder = Loess::new().fraction(0.8).iterations(5);

    // Create a new default builder
    let default_builder = Loess::<f64>::new();
...
fn test_adapter_type_inference() {
    ...
    let _batch_processor = Loess::new().fraction(0.5).adapter(Batch).build().unwrap();
